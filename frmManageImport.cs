...
namespace Pharmacy_Nhom1
{
    public partial class frmManageImport : Form
    {
        ...
        private void frmManageImport_Load(object sender, EventArgs e)
        {
            LoadData();
        }
        ...
        private void btNew_Click(object sender, EventArgs e)
        {
            ...
            {
                mainForm.OpenChildForm(new frmNewImport());
            }
            else
            {
                frmNewImport inputForm = new frmNewImport();
        ...
        private void dgvImports_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvImports.Rows[e.RowIndex].DataBoundItem is ImportViewModel currentItem)
            {
                ...
                if (columnName == "Edit")
                {
                    if (this.MdiParent is frmMain mainForm)
                    {
                        mainForm.OpenChildForm(new frmEditImport(importId));
                    }
                    else
                    {
                        frmEditImport editForm = new frmEditImport(importId);
