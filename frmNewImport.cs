...
{
    public partial class frmNewImport : Form
    {
        ...
        // Danh sách đơn vị sỉ cố định cho cboWholesaleUnit
        private static readonly string[] WholesaleUnitOptions =
            { "Hộp", "Thùng", "Chai", "Lọ", "Tuýp", "Gói", "Túi", "Bình", "Ống", "Cái", "Bịch" };

        // ─────────────────────────────────────────────────────────────────────
        // Wrapper cho bảng chi tiết phiếu nhập (dgvImportDetails)
        // ─────────────────────────────────────────────────────────────────────
        public class ImportDetailWrapper
        {
            ...
            /// <summary>Đơn vị sỉ được chọn khi nhập (Hộp, Thùng, Chai...)</summary>
            public string WholesaleUnitName { get; set; } = string.Empty;

            /// <summary>Số lượng đơn vị sỉ nhập về</summary>
            public int InputQuantity { get; set; }

            /// <summary>Giá nhập / đơn vị sỉ</summary>
            ...
            public string LotStatusDisplay => $"Hợp lệ ({WholesaleUnitName})";
            public string ConversionRatioDisplay { get; set; } = "—";
            public string DisplayProductName => string.IsNullOrEmpty(WholesaleUnitName)
                ? ProductName
                : $"{ProductName} ({WholesaleUnitName})";
        }
        ...
        {
            // Thiết lập dgvProductUnits
            dgvProductUnits.AutoGenerateColumns = false;

            // Thiết lập dgvImportDetails
            ...
            txtImportCode.Text = "PNK-" + DateTime.Now.ToString("yyyyMMddHHmmss");
            txtCreatedBy.Text = Utility.CurrentUser?.FullName ?? "Dược sĩ Lê Minh Tâm (Chủ tiệm)";
        ...
        private void LoadProductConversionTable()
        {
            ...
            var bindingList = new BindingList<ProductUnit>(_uiProductUnitsList);
            dgvProductUnits.DataSource = bindingList;
        ...
        /// <summary>Khi tỷ lệ quy đổi bị chỉnh tay → tính lại giá ngay</summary>
        ...
        private void btAddDetail_Click(object sender, EventArgs e)
        {
            ...
            // Kiểm tra trùng lô (cùng sản phẩm, cùng số lô, cùng đơn vị sỉ)
            ...
            else
            {
                _importDetailsGridList.Add(new ImportDetailWrapper
                {
                    ...
                    ConversionRatioDisplay = wholesaleUnit,
                    InputQuantity = qty,
        ...
        private void btSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var db = new PharmacyDbContext())
                {
                    var importToSave = new Import { ImportCode = txtImportCode.Text.Trim() };
                    ...
                    importToSave.Status = cboStatus.SelectedValue != null
                        ? Convert.ToByte(cboStatus.SelectedValue) : (byte)1;
                    ...
                    foreach (var wrapper in _importDetailsGridList)
                    {
                        var detail = new ImportDetail
                        {
                            ...
                            ImportPrice = wrapper.InputPrice   // giá/đơn vị sỉ nhập
                        };
                    ...
                    foreach (var uiUnit in _uiProductUnitsList)
                    {
                        ...
                        if (dbUnit != null)
                        {
                            dbUnit.ConversionRate = newConversionRate;
                            // Chỉ ghi ImportPrice, KHÔNG ghi đè Price (giá bán do dược sĩ tự set)
            ...
            {
                MessageBox.Show("Xảy ra lỗi khi lưu: " + ex.Message, "Lỗi hệ thống", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
